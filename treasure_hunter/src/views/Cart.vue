...
getData:function(data){
    ...
    if(data) { // If the user is logged in and data is sent
        this.nickName = data.a;
    }else{ // User is not logged in and needs to be notified to do so
...
alterOne(productId,flag){
    if(!this.nickName){
        alert('You are not logged in, please signup or login'+this.nickName);
        return};
    axios.post("/items/alterOne",{
        fullname:this.nickName,
        productId:productId,
